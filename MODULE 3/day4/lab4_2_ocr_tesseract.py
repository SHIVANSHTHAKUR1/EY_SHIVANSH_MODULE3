...
def ensure_tesseract_configured() -> bool:
    ...
    if custom_cmd and Path(custom_cmd).exists():
        ...
        print(f"[INFO] Using Tesseract from TESSERACT_CMD: {custom_cmd}")
        return True
    ...
    if platform.system() == "Windows":
        ...
        for candidate in common_paths:
            if Path(candidate).exists():
                ...
                print(f"[INFO] Detected Tesseract at: {candidate}")
                return True
...
if not ensure_tesseract_configured():
    ...
    if platform.system() == "Windows":
        ...
        print("        After install, set TESSERACT_CMD env var or update pytesseract path above.")
    else:
        print("        Install via your package manager (e.g., brew install tesseract).")
    sys.exit(1)
...
try:
    ...
    print(f"[ERROR] Unable to communicate with Tesseract: {exc}")
    sys.exit(1)
