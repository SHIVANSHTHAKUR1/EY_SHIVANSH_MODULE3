...
# =============================================================================
# SECTION 7: INTERACTIVE LAB MENU
# Provides a text-based menu so students can run each phase sequentially.
# =============================================================================
...
"""
    ...
    while True:
        ...
        if choice in {"6", "q", "quit", "exit"}:
            print("[INFO] Exiting lab workflow. Great job!")
            break

        if choice == "1":
            names_input = input("Enter comma-separated person names: ").strip()
            if not names_input:
                print("[WARN] No names provided. Try again.")
                continue
            try:
                num_photos = int(input("Photos per person (default 5): ") or 5)
            except ValueError:
                num_photos = 5
                print("[WARN] Invalid number provided. Defaulting to 5 photos per person.")

            for raw_name in names_input.split(','):
                name = raw_name.strip()
                if not name:
                    continue
                capture_face_photos(name, max(1, num_photos))

        elif choice == "2":
            img1 = input("Path to image 1: ").strip().strip('"')
            img2 = input("Path to image 2: ").strip().strip('"')
            if not (os.path.exists(img1) and os.path.exists(img2)):
                print("[ERROR] One or both image paths are invalid. Please try again.")
                continue
            verify_two_faces(img1, img2)

        elif choice == "3":
            query = input("Path to query image: ").strip().strip('"')
            if not os.path.exists(query):
                print("[ERROR] Query image not found. Please provide a valid path.")
                continue
            identify_face(query)

        elif choice == "4":
            print("[INFO] Launching real-time recognition. Press 'q' to stop in the video window.")
            run_realtime_recognition()

        elif choice == "5":
            folder = input("Path to labelled test folder (default ./test_images): ").strip() or "./test_images"
            evaluate_accuracy(folder)

        else:
            print("[WARN] Invalid option. Please choose between 1 and 6.")
